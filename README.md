...
项目已包含 `.gitignore`、MIT `LICENSE`、`pyproject.toml` 和跨平台 GitHub Actions。创建空的 GitHub 仓库后执行：
...
`downloads/`、虚拟环境、日志和缓存已被忽略，不会随正常提交上传。公开仓库前仍应检查暂存文件，确保没有 cookies、个人路径或未获授权的视频。
