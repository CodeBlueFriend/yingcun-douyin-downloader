...
程序会自动打开 `http://127.0.0.1:8000`。Web 页面只在当前电脑上运行，视频默认保存到项目的 `downloads` 目录。你可以在“保存目录”中输入相对路径、绝对路径或 `~/` 路径，并点击“打开目录”调用 Finder、Explorer 或系统文件管理器。下载时按钮会显示“正在下载”，成功后弹窗展示完整保存位置。使用结束后回到终端按 `Ctrl+C` 停止。
